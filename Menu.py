...
class Menu:
    ...
    def lanzarMenu(self,manejadorViajero):
        ...
        while bandera:
            ...
            if manejadorViajero.buscarViajeroPorNumero(numeroViajero)!=-1:
                bandera=False
            else:
        ...
        while(i!=opcion):
            ...
            if opcion=='1' or opcion=='2' or opcion=='3':
                ejecutar(manejadorViajero,numeroViajero)
            elif opcion=='4':
                ejecutar(manejadorViajero)
            else:
                ejecutar()
    def opcion1(self,manejadorViajero,numeroViajero):
        manejadorViajero.consultarMillas(numeroViajero)
    def opcion2(self,manejadorViajero,numeroViajero):
        millas=int(input('Ingrese las millas para acumular:\n'))
        manejadorViajero.acumularMillas(numeroViajero,millas)
    def opcion3(self,manejadorViajero,numeroViajero):
        millas=int(input('Ingrese las millas para canjear:\n'))
        if millas<0:
            print('No se admiten valores negativos')
        else:
            manejadorViajero.canjearMillas(numeroViajero,millas)
